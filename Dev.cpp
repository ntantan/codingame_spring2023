...
void	set_cells_value()
{
	...
	if (cellList.potential_ants % 2 == 0 && cellList.ally_ants >= cellList.potential_ants / 2 
		|| cellList.potential_ants % 2 == 1 && cellList.ally_ants >= (cellList.potential_ants + 1) / 2)
	{
		egg_value = 0;
		crystal_value = 100;
		cellList.egg_phase = false;
	}
	else
	{
		cellList.egg_phase = true;
		egg_value = 30;
	}
	...
	for (int i = 0; i < cells.size(); i++)
	{
		...
		if (cellList.egg_phase && cells[i].type == EGG)
		{
			if (dist_min_ally > dist_min_oppo)
				...
				dist_value = 20 + dist_min_ally;
			else
				dist_value = 60 - dist_min_ally;
		}
		else
		{
...
int	find_max_target(vector<Cell> cells)
{
	int max_value = 0;
	int max_target = 0;
	...
	for (int i = 1; i < cells.size(); i++)
	{
		...
		vector<Cell>	cells_cpy(cells.begin(), cells.begin() + i);
		for (int i = 0; i < cellList.ally_base_indexes.size(); i++)
			linked_cell_indexes.push_back(cellList.ally_base_indexes[i]);

		sort_cells_by_distance(cells_cpy);
		for (vector<Cell>::iterator it = cells_cpy.begin(); it != cells_cpy.end();)
		{
		...
		int	current_value = 0;
		for (int fp = 0; fp < full_path.size(); fp++)
			current_value += cellList.cells[full_path[fp]].value;
		int	value_per_cell = current_value / full_path.size();
		if (value_per_cell >= max_value)
		{
			max_value = value_per_cell;
			max_target = i + 1;
		}
		// cerr << "MAX TARGET " << i << " VALUE " << value_per_cell << endl;
	}
	return (max_target);
}
...
vector<vector<Cell>>	generate_priority_cells()
{
	...
	int max_target = find_max_target(cells_for_base);
	...
	{	
		if (target_cell[i].ally_ants <= target_cell[i].oppo_ants
			&& target_cell[i].ally_ants > 0
			&& target_cell[i].oppo_ants > 0)
		{
			cerr << "CELL " << target_cell[i].self_index << " IN DANGER" << endl;
			threats += 1;
		}
	}
	if (threats < target_cell.size())
	{
		for (int j = target_cell.size(); j > 0 && threats > 0; j--)
		{
			if (target_cell[j].type == CRYSTAL)
			{
				cerr << "FF " << target_cell[j].self_index << endl;
				target_cell.erase(target_cell.begin() + j);
				threats--;
			}
		}
	}
